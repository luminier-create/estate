...
| 주변 POI | `cache_poi` | `카테고리:좌표4자리:반경` | 90일 | 시설 변동 느림 |
| 대중교통 경로 | `cache_route` | `출발4자리:도착4자리:수단` | 30일 | 노선 개편 주기 |
| 실거래가 | `cache_market` | `법정동:YYYYMM` | 최근 1개월 1일 / 2개월 7일 / 그 이전 무기한 | 신고 기한(계약일+30일) 동안 계속 채워짐 |
| 분석 결과 | `analyses` | `prop:profileHash:preset` | 7일 | 프로필 변경 시 무효화 |

좌표는 소수 4자리(약 11m)로 뭉쳐 인접 좌표가 같은 캐시를 쓰게 한다.
캐시 계층에서 발생한 오류는 삼키고 원본 호출로 진행한다 — 캐시 문제로 기능이 죽으면 안 된다.

**쿼터 보호**: 일일 호출 카운터를 `quota/{provider}_{YYYYMMDD}` 에 기록한다.
캐시에 적중하면 쿼터를 소모하지 않으며, 모의 구현으로 동작 중일 때도 소모하지 않는다.
상한 도달 시 `QuotaExceededError` 를 던져 해당 축만 결측 처리되고,
설정 화면에 잔여 한도를 표시한다.

기본 상한(환경변수 `QUOTA_KAKAO` / `QUOTA_MOLIT` / `QUOTA_ODSAY` 로 조정):

| Provider | 기본 상한 | 근거 |
|---|---|---|
| 카카오 로컬 | 30,000 | 일반 호출 한도 대비 여유 |
| 국토부 실거래가 | 9,000 | 개발계정 일 10,000회 대비 여유 |
| ODsay | 800 | 무료 티어 상한이 콘솔 확인 대상이라 보수적으로 설정 |
